...
store that with `skyrim_case_evidence_import`, or have the helper drop JSON/TXT
files into the case `incoming` folder and call `skyrim_case_inbox_import`, so
the case remains append-only and auditable.
