...
Write-Host "  $DirectPython `"$Server`" --skyrim-diagnostics"
Write-Host "  $DirectPython `"$Server`" --safe-session"
Write-Host "  $DirectPython `"$Server`" --tool validate_setup"
Write-Host "  $DirectPython `"$Server`" --tool detect_environment"
Write-Host "  $DirectPython `"$Server`" --tool nexus_validate_key"
Write-Host "  $DirectPython `"$Server`" --tool in_game_issue_report --description `"annoying popup after loading a save`""
...
Write-Host "Nexus API metadata is optional. Set NEXUS_MODS_API_KEY or use --nexus-api-key-file; MCP Doctor does not copy Vortex's key."
